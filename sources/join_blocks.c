...
mem_block *join_blocks (mem_block *left_block)
{
    ...
    {
        if(right_block -> fprev != NULL)
            right_block -> fprev -> fnext = right_block -> fnext;
        if(right_block -> fnext != NULL)
            right_block -> fnext -> fprev = right_block -> fprev;
        
        if(mem_blocks_fhead == right_block)
            mem_blocks_fhead -> fnext = right_block -> fnext;
        
        if(mem_blocks_ftail == right_block)
            mem_blocks_ftail -> fprev = right_block -> fprev;
    }
